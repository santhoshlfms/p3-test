...
var consumerOnboardingData = JSON.stringify({
  person_details: {
    ...
    email_addresses: [
      {
        email_address: "app2apptest@pp.com",
        primary: true,
        confirmed: true,
      },
    ],
...
const consumerReferralApi = (requestOptions: any, cb: any) => {
  ...
  fetch(
    "https://api.sandbox.paypal.com/v1/customer/consumer-referrals",
    requestOptions
  )
    .then((res) => res.json())
    .then(
...
const linkedInstrumentApi = (data: any, cb: any) => {
  ...
  fetch(
    "https://api.sandbox.paypal.com/v1/payment-networks/linked-instruments",
    requestOptions
  )
    .then((res) => res.json())
    .then(
